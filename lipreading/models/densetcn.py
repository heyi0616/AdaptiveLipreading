    ...
    def __init__(self, block_config, growth_rate_set, input_size, reduced_size,
        ...
        for i, num_layers in enumerate(block_config):
            ...
            if i != len(block_config) - 1:
                ...
                num_features = reduced_size
                # self.features.add_module('adapter%d' % (i + 1), Adapter(num_features, reduction=8))
                # if i == 0:
                # encoder_layer = transformer_torch.TransformerEncoderLayer(d_model=reduced_size, nhead=8, dim_feedforward=128, dropout=0)
                # encoder = transformer_torch.TransformerEncoder(encoder_layer, num_layers=1)
                # self.features.add_module('avg_adapter%d' % (i+1), AdaptAvg(num_features))
            # else:
            #     self.features.add_module('avg_adapter%d' % (i+1), AdaptAvg(num_features))
        # Final batch norm
