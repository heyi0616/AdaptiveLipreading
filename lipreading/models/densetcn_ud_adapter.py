...
from lipreading.models import transformer_torch
...
class _ConvBatchChompRelu(nn.Module):
    def __init__(self, n_inputs, n_outputs, kernel_size_set, stride, dilation, dropout, relu_type, se_module=False, depth=0):
...
class _DenseBlock(nn.ModuleDict):
    ...
    def __init__( self, num_layers, num_input_features, growth_rate,
                  ...
                  dropout, relu_type, squeeze_excitation, depth
                  ):
...
class _Transition(nn.Sequential):
    def __init__(self, num_input_features, num_output_features, relu_type):
        ...
        if num_input_features != 512:
            self.add_module('time_adapter', AdaptAvg(num_output_features))
...
class AdaptAvg(nn.Module):
    def __init__(self, length):
        super(AdaptAvg, self).__init__()
    ...
    def forward(self, x):
        attn = self.proj(x.transpose(1, 2))
        attn = F.sigmoid(attn) * 2
        attn = (x.transpose(1, 2) * attn).transpose(1, 2)  # (B, C, T)
        return attn
