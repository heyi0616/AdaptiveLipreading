...
class MyDataset(object):
    ...
    def load_dataset(self):
        ...
        for i, x in enumerate(self._data_files):
            ...
            instance_ids[i] = self._get_instance_id_from_path(x)
    ...
    def _get_files_for_partition(self):
        ...
        if self._data_partition == "train":
            self._data_files = [f for f in self._data_files if int(f.split('_')[-1].split('.')[0]) < 250]
    ...
    def load_data(self, filename):
        try:
            ...
            print(f"Error when reading file: {filename}, msg: {str(e)}")
            sys.exit()
