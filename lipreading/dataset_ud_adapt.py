...
class AdaptDataset(Dataset):
    def __init__(self, modality, data_partition, data_dir, label_fp, subject, adapt_min, fold=1, annonation_direc=None,
                 preprocessing_func=None, data_suffix='.npz', use_boundary=False, specific_word=None):
        assert os.path.isfile(label_fp), \
            f"File path provided for the labels does not exist. Path iput: {label_fp}."
        assert data_partition in ["train", "val", "test"]
        ...
        assert adapt_min in [0, 1, 3, 5]
        self.adapt_min = adapt_min
        self.fold = fold
        self.logger = logging.getLogger("mylog" + str(fold))
        ...
        subject_list = [4243, 5125, 6003, 7184, 9335, 9368, 9438, 9653, 10209, 10293, 10587, 11041, 11777, 11875,
                        11910, 13287, 13786, 15545, 15769, 17378]
        self.subject_id = subject_list[subject]
    ...
    def load_dataset(self):
        ...
        data_partition = "val" if self._data_partition == "train" else self._data_partition
        with open(split_file + 'LRW_ID_{}.txt'.format(data_partition), 'r') as f:
            lines = f.readlines()
        for l in lines:
            subject, f_name = l.strip().split()
            if int(subject) == self.subject_id:
        ...
        if self._data_partition == 'train':
            file_list = file_list[:round(self.adapt_min * 60 / 1.16)]
            print(
                f'subject index:{self.subject}, {self._data_partition} with fold {self.fold}, Use data of {self.adapt_min} min, so {len(file_list)} data')
            self.logger.info("fold " + str(self.fold) + " train file list: " + str(["/".join(f.split("/")[-2:]) for f in file_list]))
        elif self._data_partition == 'val':
            file_list = file_list[round(self.adapt_min * 60 / 1.16):]
            print(
                f'subject index:{self.subject}, {self._data_partition} with fold {self.fold}, Validate on remaining adaptation set, so {len(file_list)} data')
        else:
            # print(f'subject index:{self.subject}, {self._data_partition}, Use full data, so {len(file_list)} data')
            pass
        if self.specific_word is not None:
            file_list = list(filter(lambda x: self._get_label_from_path(x) in self.specific_word, file_list))
    ...
    def __getitem__(self, idx):
        ...
        if (self._data_partition == 'train') and self.is_var_length and not self.use_boundary:
            ...
            data = raw_data
        preprocess_data = self.preprocessing_func(data)  # (T, H, W)
